...
import { createVisualisationBrief } from '@/app/admin-dashboard/visualisation-actions';
...
export default function VisualisationBriefStudio({ projectSlug, spaces, moodboards, concepts, assets, visualisations }: { projectSlug: string; spaces: Space[]; moodboards: Moodboard[]; concepts: Concept[]; assets: Asset[]; visualisations: Visualisation[] }) {
  ...
  return <section className="grid gap-6 lg:grid-cols-[360px_1fr]">
    <aside className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm">
      ...
      <p className="mt-1 text-sm text-stone-500">The renderer will receive a controlled brief instead of an unconstrained prompt.</p>
    ...
    <div className="space-y-6">
      ...
      <div className="rounded-3xl border border-stone-200 bg-white p-6 shadow-sm"><div className="flex items-center justify-between"><h3 className="text-lg font-semibold text-stone-900">Visualisation jobs</h3><span className="text-xs text-stone-400">{visualisations.length} briefs</span></div><div className="mt-4 space-y-2">{visualisations.length ? visualisations.map(v=><div key={v.id} className="flex items-center justify-between rounded-2xl border border-stone-100 bg-stone-50 p-4"><div><p className="text-sm font-medium text-stone-800">{v.name}</p><p className="mt-1 text-xs text-stone-400">{new Date(v.created_at).toLocaleString()}</p></div><span className="rounded-full bg-white px-3 py-1 text-xs font-medium text-stone-600">{v.status}</span></div>) : <p className="py-8 text-center text-sm text-stone-400">No visualisation briefs yet.</p>}</div></div>
    </div>
  </section>;
}
