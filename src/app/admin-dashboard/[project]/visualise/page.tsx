...
export default async function VisualisePage({ params }: { params: Promise<{ project: string }> }) {
  ...
  return <main className="min-h-screen bg-stone-50 px-6 py-10"><div className="mx-auto max-w-7xl">
    ...
    <VisualisationBriefStudio projectId={project.id} projectSlug={slug} spaces={spaces ?? []} moodboards={moodboards ?? []} concepts={concepts ?? []} assets={signedAssets} visualisations={visualisations ?? []} renderAssets={renderAssets} />
  </div></main>;
}
