...
export default async function DesignStudioPage({ params }: { params: Promise<{ project: string }> }) {
  ...
  let assets: Asset[] = [];
  let moodboard: any | null = null;
  if (dbProject) {
    const [{ data: members }, { data: dbSpaces }, { data: dbDesigns }, { data: dbAssets }, { data: dbMoodboard }] = await Promise.all([
      ...
      supabase.from('moodboards').select('id, project_space_id, name, description, style_direction, palette, notes, updated_at').eq('project_id', dbProject.id).neq('status', 'archived').order('updated_at', { ascending: false }).limit(1).maybeSingle(),
    ]);
