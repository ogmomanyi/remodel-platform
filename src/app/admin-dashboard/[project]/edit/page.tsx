...
export default async function DesignStudioPage({ params }: { params: Promise<{ project: string }> }) {
  ...
  const project = dbProject ? {
    ...
    description: String(legacyProject!.frontmatter.description || ''),
    allowed_emails: legacyProject!.allowed_emails,
    materials_required: legacyProject!.materials_required,
  };

  let emails = project.allowed_emails;
  let spaces: Array<{ id: string; name: string; space_type: string }> = [];
  if (dbProject) {
    const [{ data: members }, { data: dbSpaces }] = await Promise.all([
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <section className="lg:col-span-2 space-y-6">
            ...
            <div className="bg-white rounded-xl border border-gray-200 p-6">
              <div className="flex items-center justify-between mb-5"><div><h2 className="text-lg font-semibold">Spaces & design options</h2><p className="text-sm text-gray-500 mt-1">Build the renovation area by area.</p></div><span className="text-xs rounded-full bg-slate-100 px-3 py-1">{spaces.length} spaces</span></div>
              {spaces.length ? <div className="grid sm:grid-cols-2 gap-4">{spaces.map((space) => <div key={space.id} className="rounded-lg border border-gray-200 p-5"><h3 className="font-medium text-gray-900">{space.name}</h3><p className="text-sm text-gray-500 mt-1">{space.space_type}</p></div>)}</div> : <div className="rounded-lg border border-dashed border-gray-300 p-8 text-center"><h3 className="font-medium text-gray-800">No spaces added yet</h3><p className="text-sm text-gray-500 mt-2">The next editor layer will let you add rooms, patios, verandas and other areas with measurements and site photos.</p></div>}
            </div>
