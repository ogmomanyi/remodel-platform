...
export default async function ProcurementPage({ params }: { params: Promise<{ project: string }> }) {
  ...
  return <main className="min-h-screen bg-stone-50 px-6 py-10"><div className="mx-auto max-w-7xl">
    <div className="mb-8 flex items-start justify-between gap-4"><div><p className="text-xs font-semibold uppercase tracking-[0.2em] text-stone-500">{project.project_code} · {project.client_name}</p><h1 className="mt-1 text-3xl font-semibold tracking-tight text-stone-900">Procurement</h1><p className="mt-1 text-sm text-stone-500">Turn the approved design into a controlled purchasing list.</p></div><div className="flex flex-wrap gap-2"><Link href={`/admin-dashboard/${slug}/proposal`} className="rounded-full border border-stone-300 bg-white px-4 py-2 text-sm font-medium text-stone-700">Proposal</Link><Link href={`/admin-dashboard/${slug}/procurement/orders`} className="rounded-full border border-stone-300 bg-white px-4 py-2 text-sm font-medium text-stone-700">Purchase orders</Link><Link href={`/admin-dashboard/${slug}/execution`} className="rounded-full bg-stone-900 px-4 py-2 text-sm font-medium text-white">Execution</Link></div></div>
