...
export default async function ProposalPage({ params }: { params: Promise<{ project: string }> }) {
  ...
  const builderOptions = (options ?? []).map((option) => ({
    ...
    costEstimate: option.cost_estimate,
    materialCost: buildBom(Array.isArray(option.materials) ? option.materials : []).material_total,
  }));
