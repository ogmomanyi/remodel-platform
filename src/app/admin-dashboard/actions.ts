...
type MoodboardRecord = { id: string; project_space_id: string | null; name: string; style_direction: string; palette: unknown; description: string | null; notes: string | null };
type DesignConceptRecord = { id: string; project_space_id: string | null; name: string; elements: unknown };
type SourceAssetRecord = { id: string; space_id: string | null; kind: string; storage_path: string; alt_text: string | null; metadata: Record<string, unknown> | null };
...
function normaliseMaterials(materials: MaterialReference[] | undefined) { if (!Array.isArray(materials)) return []; return materials.slice(0, 100).map((item) => ({ category: item.category, material: String(item.material || '').trim().slice(0, 200), supplier: String(item.supplier || '').trim().slice(0, 160), product_reference: String(item.product_reference || '').trim().slice(0, 160), quantity: item.quantity == null ? null : Number(item.quantity), unit: String(item.unit || '').trim().slice(0, 40), estimated_material_cost: item.estimated_material_cost == null ? null : Number(item.estimated_material_cost), notes: String(item.notes || '').trim().slice(0, 500) })).filter((item) => item.material); }
...
export async function createVisualisationBrief(input: { projectSlug: string; name: string; spaceId?: string | null; moodboardId?: string | null; designConceptId?: string | null; sourceAssetId?: string | null }) {
  await requireAdmin(); const name = input.name.trim(); if (!name) throw new Error('A visualisation name is required.');
  const { supabase, project } = await resolveProject(input.projectSlug);
  ...
  if (input.spaceId && !space) throw new Error('Selected space does not belong to this project.');
  let moodboard: MoodboardRecord | null = null; if (input.moodboardId) { const { data, error } = await supabase.from('moodboards').select('id, project_space_id, name, style_direction, palette, description, notes').eq('id', input.moodboardId).eq('project_id', project.id).maybeSingle(); if (error || !data) throw new Error('Moodboard not found.'); moodboard = data; }
  let concept: DesignConceptRecord | null = null; if (input.designConceptId) { const { data, error } = await supabase.from('design_concepts').select('id, project_space_id, name, elements').eq('id', input.designConceptId).eq('project_id', project.id).maybeSingle(); if (error || !data) throw new Error('Design concept not found.'); concept = data; }
  let sourceAsset: SourceAssetRecord | null = null; if (input.sourceAssetId) { const { data, error } = await supabase.from('project_assets').select('id, space_id, kind, storage_path, alt_text, metadata').eq('id', input.sourceAssetId).eq('project_id', project.id).maybeSingle(); if (error || !data) throw new Error('Source asset not found.'); sourceAsset = data; }
  const palette = Array.isArray(moodboard?.palette) ? moodboard.palette.join(', ') : '';
  const prompt = ['Create a photorealistic interior design visualisation for a renovation project.', space ? `Room: ${space.name} (${space.space_type}).` : 'Scope: whole project.', moodboard ? `Design direction: ${moodboard.style_direction}.` : '', moodboard?.description ? `Mood: ${moodboard.description}` : '', palette ? `Colour palette: ${palette}.` : '', moodboard?.notes ? `Designer notes: ${moodboard.notes}` : '', concept ? `Spatial concept: ${concept.name}. Preserve the proportions and placement implied by the supplied concept.` : '', sourceAsset ? 'Use the supplied source room/site image as the visual base. Preserve architecture, camera perspective and major openings while applying the proposed design.' : 'Use a realistic architectural photography viewpoint appropriate to the room.', 'Prioritise believable materials, natural lighting, accurate scale, realistic joinery and high-end residential styling.'].filter(Boolean).join('\n');
  ...
  await supabase.from('project_events').insert({ project_id: project.id, event_type: 'visualisation_brief_created', metadata: { visualisation_id: saved.id } }); revalidatePath(`/admin-dashboard/${project.slug}/edit`); revalidatePath(`/${project.slug}`); return saved;
}
