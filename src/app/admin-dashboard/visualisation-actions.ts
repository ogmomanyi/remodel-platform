...
export async function createVisualisationBrief(input: {
  ...
  const metadata = { source: 'kota-visualisation-studio', direction: moodboard?.style_direction || 'Modern', palette: Array.isArray(moodboard?.palette) ? moodboard.palette : [], moodboard_name: moodboard?.name || null, design_name: design?.name || null, source_asset_id: sourceAsset?.id || null };
...
export async function renderVisualisation(input: { visualisationId: string }) {
  await requireAdmin();
  const supabase = createAdminClient();
  ...
  try {
    ...
    revalidatePath(`/admin-dashboard/${job.metadata?.project_slug || ''}/visualise`);
