...
export async function createProjectTask(input: { projectSlug: string; projectSpaceId?: string; title: string; description?: string; priority?: string; assigneeName?: string; assigneeEmail?: string; startDate?: string; dueDate?: string }) {
  await requireAdmin();
  ...
  const { data: task, error } = await supabase.from('project_tasks').insert({ project_id: project.id, project_space_id: input.projectSpaceId || null, title, description: input.description?.trim() || null, priority: input.priority || 'medium', assignee_name: input.assigneeName?.trim() || null, assignee_email: input.assigneeEmail?.trim() || null, start_date: input.startDate || null, due_date: input.dueDate || null }).select('id, title, status, priority').single();
...
export async function updateProjectTask(input: { taskId: string; status?: string; priority?: string; title?: string; description?: string; assigneeName?: string; assigneeEmail?: string; startDate?: string; dueDate?: string }) {
  await requireAdmin();
  const supabase = createAdminClient();
  ...
  const patch: Record<string, unknown> = { updated_at: new Date().toISOString(), completed_at: nextStatus === 'done' ? new Date().toISOString() : null };
  if (input.status) patch.status = input.status;
  if (input.priority) patch.priority = input.priority;
  if (input.title !== undefined) { const title = input.title.trim(); if (!title) throw new Error('Task title is required.'); patch.title = title; }
  ...
  if (project) { await supabase.from('project_events').insert({ project_id: task.project_id, event_type: 'task_updated', metadata: { task_id: task.id, title: input.title?.trim() || task.title, status: nextStatus } }); revalidatePath(`/admin-dashboard/${project.slug}/execution`); }
}

export async function createProgressUpdate(input: { projectSlug: string; projectSpaceId?: string; taskId?: string; updateType?: string; title: string; notes?: string; percentComplete?: number }) {
  await requireAdmin();
  const title = input.title.trim();
  if (!input.projectSlug?.trim()) throw new Error('Project is required.');
  ...
  const { supabase, project } = await getProject(input.projectSlug);
  const { data: update, error } = await supabase.from('project_progress_updates').insert({ project_id: project.id, project_space_id: input.projectSpaceId || null, task_id: input.taskId || null, update_type: input.updateType || 'site_visit', title, notes: input.notes?.trim() || null, percent_complete: Math.round(percent) }).select('id, title, percent_complete, created_at').single();
