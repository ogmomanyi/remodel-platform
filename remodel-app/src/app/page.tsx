...
export default async function ClientDashboard() {
  ...
  const accessibleProjects: Array<{ slug: string; client_name: string; project_code: string; status: string }> = [];
  ...
  const projectCodes = accessibleProjects.map((project) => project.project_code);
  ...
  approvalRecords?.forEach((record: any) => {
      ...
      <div className="max-w-5xl mx-auto">
        <header className="flex justify-between items-center mb-12 pb-6 border-b border-gray-200">
          ...
          <form action="/auth/signout" method="POST">
            <button className="text-sm font-medium text-gray-600 hover:text-black border border-gray-300 px-4 py-2 rounded transition">
              Sign Out
            </button>
          </form>
        </header>
            ...
            {projectsWithLiveStatus.map((project) => (
              <Link href={`/${project.slug}`} key={project.slug}>
                <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200 hover:shadow-md transition cursor-pointer group">
                  ...
                  <p className="text-sm text-gray-500 font-medium">View Proposal →</p>
                </div>
              </Link>
            ))}
          </div>
